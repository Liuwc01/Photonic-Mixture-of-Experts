...
class _SharedBackend(nn.Module):
    def __init__(self, in_channels=8, shared_conv_channels=SHARED_CONV_CHANNELS, shared_fc_dim=SHARED_FC_DIM):
        ...
        with torch.no_grad():
            feat_size = self.shared_conv(torch.zeros(1, in_channels, FEATURE_MAP_SIZE, FEATURE_MAP_SIZE)).numel()
...
class WeightedRoutingPMoE(_SharedBackend):
    ...
    def __init__(self):
        super().__init__(in_channels=8)
        self.input_scales = nn.Parameter(torch.ones(3, 3))
    ...
    def forward(self, x, task_id, channel_indices=None):
        x = self.optical_matrix(x)
        x = self.adapters[task_id](x)
        if channel_indices is not None:
            scale_row = {0: 2, 1: 0, 2: 1}[task_id]
            scales = self.input_scales[scale_row, channel_indices].view(-1, 1, 1, 1)
            x = x * scales
        return self.forward_backend(x, task_id)
